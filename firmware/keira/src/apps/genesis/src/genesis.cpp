...
static constexpr int AUDIO_BUFFER_LENGTH = GWENESIS_AUDIO_BUFFER_LENGTH_PAL;//**std::max(GWENESIS_AUDIO_BUFFER_LENGTH_NTSC, GWENESIS_AUDIO_BUFFER_LENGTH_PAL);

static constexpr size_t GENESIS_SCREEN_WIDTH = 240;
static constexpr size_t GENESIS_VISIBLE_HEIGHT = 280;
...
void /*IRAM_ATTR*/ run_genesis_rom() {
  ...
  frameskip = sound_enabled ? full_frameskip : muted_frameskip;
    ...
    const bool keys[] = {
      (bool)state.up.pressed,
      (bool)state.down.pressed,
      (bool)state.left.pressed,
      (bool)state.right.pressed,
      (bool)state.c.pressed, // in genesis, C is the far right, so we'll make it Y
      (bool)state.a.pressed, // in genesis, B is the middle, so we'll make it A
      (bool)state.b.pressed, // in genesis, A is the far left, so we'll make it B
      (bool)state.start.pressed,
    };
